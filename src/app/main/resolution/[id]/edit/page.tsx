...
export default function EditarResolucionPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const resolucionData = {
        ...
        descripcion:
          "Resolución que autoriza las frecuencias de transporte interprovincial para la ruta Ambato-Quito y viceversa.",
        activo: true,
        documentoUrl: "/documents/ant-2025-001.pdf",
      }

      setFormData({
        numeroResolucion: resolucionData.numeroResolucion,
        fechaEmision: resolucionData.fechaEmision,
        fechaVigencia: resolucionData.fechaVigencia,
        descripcion: resolucionData.descripcion,
        activo: resolucionData.activo,
      })
      setCurrentDocumentUrl(resolucionData.documentoUrl)
      setIsLoading(false)
    }, 1000)
  }, [params.id])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Nuevo archivo:", documentFile)

      // Redirigir al detalle de la resolución
    //  window.location.href = `//resoluciones/${params.id}`
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-500">Cargando resolución...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="outline" size="icon" asChild>
              <a href={`/resoluciones/${params.id}`}>
                <ArrowLeft className="h-4 w-4" />
              </a>
            </Button>
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Editar Resolución</h1>
              <p className="text-gray-500">{formData.numeroResolucion}</p>
            </div>
          </div>
        </div>
      </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Actualizar el archivo PDF de la resolución oficial</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {currentDocumentUrl && (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">Archivo PDF existente</p>
                        </div>
                      </div>
                ...
                <div className="space-y-2">
                  <Label htmlFor="documento">Nuevo Archivo PDF (opcional)</Label>
                  <Input
                    id="documento"
                    type="file"
                    accept=".pdf"
                  ...
                  {documentFile && (
                    <p className="text-sm text-green-600">Nuevo archivo seleccionado: {documentFile.name}</p>
                  )}
                  <p className="text-xs text-gray-500">
                    Solo archivos PDF, máximo 10MB. Si no seleccionas un archivo, se mantendrá el documento actual.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Configurar el estado de la resolución</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Alert>
              ...
              <AlertDescription>
                <strong>Nota:</strong> Los cambios se aplicarán inmediatamente. Asegúrate de que toda la información sea
            ...
            <div className="flex justify-end gap-4">
              <Button type="button" variant="outline" asChild>
                <a href={`/main/resolution/${params.id}`}>Cancelar</a>
              </Button>
